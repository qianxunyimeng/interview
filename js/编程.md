...
普通实现
不考虑非数字的情况，只实现基础版本(0.0.0这种)
将两个版本号通过"."分割为数组，将较短的数组用0补齐。最后循环遍历比较数组的每一位的大小，大或小可直接返回最终答案。循环结束之后，若仍没返回结果，则返回相等即为0。
...
function dom2json(node) {
  ...
  // 处理子节点,childNodes和children的区别就是 childNodes获取所有子节点(元素节点、文本节点、注释节点等等)，而children只会返回所有的元素节点
...
Function.prototype._call = function(ctx,...args){
  // 如果 ctx 为null 或 undefined,将上下文对象指向window
  const _self = ctx == null ? window : Object(ctx)
  // 创建独一无二的属性保存this
  const key = Symbol()
  _self[key] = this
  // 立即执行原函数，获取执行结果
  const result = _self[key](...args)
  delete _self[key]
  return result
}
```
...
```javascript
 Function.prototype._apply = function(ctx, array = []) {
  ...
  const _self = ctx == undefined ? window : Object(ctx)
  //给context新增一个独一无二的属性以免覆盖原有属性
  const key = Symbol()
  _self[key] = this
  const result = _self[key](...array)
  delete _self[key]
  return result
}
```
...
router.get("/token.jsonp",async function(ctx,next){
    ...
    adapter: (config) => {
      if(config.params.isJsonp){
        return new Promise((resolve,reject) => {
            ...
            .join('&')
            const script = document.createElement('script');
            script.src = `${config.url}?${queryString}`;
...
export const jsonpAxios = service
```
